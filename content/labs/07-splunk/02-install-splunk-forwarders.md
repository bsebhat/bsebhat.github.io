...
## Send SplunkForwarder file to juicero
Because the `juicero` is in the `DMZ` network, there are no firewall rules allowing it to download software from the internet. To install the  SplunkForwarder software that can forward the access logs to the `splunk` VM, I'll download the software onto the `sysadmin` VM, and use the secure file tool `scp` to upload the file to `juicero`:
```
scp <splunk-forwarder-file> vmadmin@juicero:/home/vmadmin
```

Then, I'll SSH into `juicero` and install SplunkForwarder.
```
...
I create an index called `juicero`. This will have the access logs from the `juicero` web server:
...
## Allow juicero To Forward Logs Out Of DMZ
In order to for the `pfsense` web server in the `DMZ` network to forward its logs to the `splunk` server, I need to add a firewall rule. I'll make the rule specific: allow TCP traffic from the source host `juicero` (IP `192.168.2.21`) to the destination `splunk` server (IP `192.168.3.10`) at the destination's `9997` port. That's the receiving port I created on the Splunk server.
...
This tells the Splunk Forwarder to monitor the logs at `/opt/juice-shop/logs/access*` for changes and forward them to the Splunk index `juicero`. It also gives the `soucetype`, so that Splunk knows the format the logs will be in.
...
I enable the `SplunkForwarder` service on the `juicero` server:
```
...
Next, use the web app on `juicero` from a browser. Just refresh it once or twice. Then, I go on the Splunk web interface and use the search app to search for `index="juicero"` and I see access logs for the web app on `juicero`.
