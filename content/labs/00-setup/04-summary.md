...
In the [next lab](../../01-juice-shop), I'll create a new VM called `juicero`. I'll install the [OWASP Juice Shop vulnerable web application](https://owasp.org/www-project-juice-shop/) on the `juicero` VM, and create a [systemd service](https://www.freedesktop.org/software/systemd/man/latest/systemd.service.html) so that it starts whenever I boot the `juicero` VM.