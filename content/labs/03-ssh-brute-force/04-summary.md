...
I was able to use the `hydra` login cracking tool brute force the SSH service on the `juicero` server. It used a giant text document called a "wordlist". It has the most commonly used passwords, and can be used to automate the process of guessing passwords for an account. It was for a user account with an extremely weak password, near the top of many password dictionaries, so it was easily cracked.
