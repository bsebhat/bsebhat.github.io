...
Now that the `juicero` VM is acting as a web server, providing access to the Juice Shop web app, I'll create another CentOS workstation VM to represent a customer or user who uses the website to order juice products.
...
After starting up the `customer` VM, and opening a web browser, I clear all the history data. I don't want to use the saved history cloned from the `sysadmin` VM. Then, I go to `http://juicero:3000` and create a new account called `customer@example.com`, and order some apple juice.
