...
Within a few seconds, it should say it's running on port 3000. I can go on the `sysadmin` VM and access it with the URL `http://juiceshop:3000` or `http://192.168.122.10:3000`. I could also access it from my host machine at `http://192.168.122.10:3000`.
...
After submitting feedback, you can view it by logging in using the built-in administrator account. The username is `admin@juice-sh.op` and the password is `admin123`. Go to the admin panel at `http://juiceshop/#/administration` at the end of the Customer Feedback section.
